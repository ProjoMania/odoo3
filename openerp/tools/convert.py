...
form: module.record_id""" % (xml_id,)
    ...
    def _tag_workflow(self, cr, rec, data_node=None, mode=None):
        ...
        openerp.workflow.trg_validate(
            uid, model, id, rec.get('action'), cr)
            ...
            for rec in n:
                if rec.tag in self._tags:
                    try:
                        ...
                        raise ParseError(misc.ustr(e), etree.tostring(rec).rstrip(), rec.getroottree().docinfo.URL, rec.sourceline).with_traceback(exc_info[2])
        return True
