...
class YamlInterpreter(object):
    ...
    def process_record(self, node):
        ...
        view_id = record.view
        if view_id and (view_id is not True) and isinstance(view_id, str):
            module = self.module
            if '.' in view_id:
    ...
    def _create_record(self, model, fields, view_info=None, parent={}, default=True):
        ...
        def process_val(key, val):
            if fg[key]['type'] == 'many2one':
                ...
                if val and isinstance(val, (list,tuple)) and isinstance(val[0], dict):
                    ...
                    val = [(0,0,x) for x in val]
            elif fg[key]['type'] == 'many2many':
                if val and isinstance(val,(list,tuple)) and isinstance(val[0], int):
                    val = [(6,0,val)]
        ...
        if view is not False:
            ...
            record_dict = dict([(key, val) for key, val in list(defaults.items()) if key in fg])
            ...
            while nodes:
                ...
                if el.tag=='field':
                    ...
                    if el.attrib['on_change'] in ('1', 'true'):
                        ...
                        # Evaluation args
                        args = [eval(x, ctx) for x in match.group(2).split(',')]
                        result = getattr(model, match.group(1))(self.cr, self.uid, [], *args)

                    for key, val in list((result or {}).get('value', {}).items()):
                        if key in fg:
