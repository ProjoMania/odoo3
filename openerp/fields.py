...
class Field(object, metaclass=MetaField):
    ...
    def __init__(self, string=None, **kwargs):
        ...
        attrs = {key: val for key, val in kwargs.items() if val is not None}
    ...
    def _setup_dependency(self, path0, model, path1):
        ...
        if head == '*':
            ...
            fields = set(model._fields.values()) - set([self])
        else:
...
class Many2one(_Relational):
    ...
    def set_class_name(self, cls, name):
        ...
        if not self.delegate:
            self.delegate = name in list(cls._inherits.values())
...
class _RelationalMulti(_Relational):
    ...
    def convert_to_write(self, value, target=None, fnames=None):
        ...
        for record in value:
            if not record.id:
                values = {k: v for k, v in record._cache.items() if k in fnames}
