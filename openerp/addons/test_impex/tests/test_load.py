...
class test_boolean_field(ImporterCase):
    ...
    def test_falses(self):
        for lang, source, value in [('fr_FR', 'no', 'non'),
                                    ('de_DE', 'no', 'nein'),
                                    ('ru_RU', 'no', 'нет'),
                                    ('nl_BE', 'false', 'vals'),
    ...
    def test_trues(self):
        ...
        self.assertEqual(result['messages'], [
            message("Unknown value '%s' for boolean field 'unknown', assuming 'yes'" % v[0],
...
class test_integer_field(ImporterCase):
    ...
    def test_nonsense(self):
        ...
        self.assertEqual(result['messages'], [{
            ...
            'record': 0,
            'field': 'value',
            'message': "'zorglub' does not seem to be an integer for field 'unknown'",
        }])
...
class test_float_field(ImporterCase):
    ...
    def test_nonsense(self):
        ...
        self.assertIs(result['ids'], False)
        self.assertEqual(result['messages'], [
            message("'foobar' does not seem to be a number for field 'unknown'")])
...
class test_required_string_field(ImporterCase):
    ...
    def test_empty(self):
        ...
        self.assertEqual(result['messages'], [message(
            "Missing required value for the field 'unknown' (value)")])
        self.assertIs(result['ids'], False)
...
class test_text(ImporterCase):
    ...
    def test_imported(self):
        s = ("Breiðskífa er notað um útgefna hljómplötu sem inniheldur "
...
class test_selection(ImporterCase):
    ...
    def test_invalid(self):
        ...
        self.assertIs(result['ids'], False)
        self.assertEqual(result['messages'], [message(
            "Value 'Baz' not found in selection field 'unknown'",
            moreinfo="Foo Bar Qux 4".split())])

        result = self.import_(['value'], [[42]])
        self.assertIs(result['ids'], False)
        self.assertEqual(result['messages'], [message(
            "Value '42' not found in selection field 'unknown'",
            moreinfo="Foo Bar Qux 4".split())])
...
class test_m2o(ImporterCase):
    ...
    def test_by_names(self):
        ...
        self.assertEqual(
            result['messages'],
            [message("Found multiple matches for field 'unknown' (2 matches)",
                     type='warning')])
        self.assertEqual(len(result['ids']), 1)
        self.assertEqual([
            (integer_id1, name1)
    ...
    def test_fail_by_implicit_id(self):
        ...
        self.assertEqual(result['messages'], [
            message("No matching record found for name '%s' in field 'unknown'" % id,
    ...
    def test_fail_id_mistype(self):
        ...
        self.assertEqual(result['messages'], [
            message("Invalid database id 'foo' for the field 'unknown'",
    ...
    def test_sub_field(self):
        ...
        self.assertEqual(result['messages'], [
            message("Can not create Many-To-One records indirectly, import "
    ...
    def test_fail_noids(self):
        ...
        self.assertEqual(result['messages'], [message(
            ...
            "in field 'unknown'", moreinfo=moreaction(
    ...
    def test_fail_multiple(self):
        result = self.import_(
            ...
            [['somename', 'somexid']])
        self.assertEqual(result['messages'], [message(
            "Ambiguous specification for field 'unknown', only provide one of "
...
class test_m2m(ImporterCase):
    ...
    def test_nonames(self):
        ...
        self.assertEqual(result['messages'], [message(
            ...
            "field 'unknown'", moreinfo=moreaction(
...
class test_o2m(ImporterCase):
    ...
    def test_name_get(self):
        s = 'Java is a DSL for taking large XML files and converting them ' \
            'to stack traces'
        result = self.import_(
            ['const', 'value'],
            [['5', s]])
        self.assertEqual(result['messages'], [message(
            "No matching record found for name '%s' in field 'unknown'" % s,
...
class test_date(ImporterCase):
    ...
    def test_invalid(self):
        ...
        self.assertEqual(result['messages'], [
            message("'not really a date' does not seem to be a valid date "
                    "for field 'unknown'",
...
class test_datetime(ImporterCase):
    ...
    def test_invalid(self):
        ...
        self.assertEqual(result['messages'], [
            message("'not really a datetime' does not seem to be a valid "
                    "datetime for field 'unknown'",
...
class test_unique(ImporterCase):
    ...
    def test_unique(self):
        ...
        self.assertEqual(result['messages'], [
            dict(message="The value for the field 'value' already exists. "
                         "This might be 'unknown' in the current model, "
                         "or a field of the same name in an o2m.",
