...
def raise_qweb_exception(etype=None, **kw):
    ...
    try:
        raise etype(original).with_traceback(tb)
    except etype as e:
    ...
    * alternatively, override :meth:`~.get_converter_for` and return an
    ...
    def get_template(self, name, qwebcontext):
        ...
        if qwebcontext.loader and name not in qwebcontext.templates:
            ...
            self.load_document(xml_doc, isinstance(name, int) and name or None, qwebcontext=qwebcontext)
...
class MonetaryConverter(osv.AbstractModel):
    ...
    def record_to_html(self, cr, uid, field_name, record, options, context=None):
        ...
        if display_currency.position == 'before':
            pre = '{symbol}\N{NO-BREAK SPACE}'
        else:
            post = '\N{NO-BREAK SPACE}{symbol}'

        return HTMLSafe('{pre}<span class="oe_currency_value">{0}</span>{post}'.format(
            formatted_amount,
            pre=pre, post=post,
        ).format(
...
class WebAsset(object):
    ...
    def stat(self):
        if not (self.inline or self._filename or self._ir_attach):
            addon = [_f for _f in self.url.split('/') if _f][0]
            try:
