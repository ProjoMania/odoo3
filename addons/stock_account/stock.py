...
class stock_picking(osv.osv):
    ...
    def _invoice_create_line(self, cr, uid, moves, journal_id, inv_type='out_invoice', context=None):
        ...
        for move in moves:
            ...
            if key not in invoices:
                ...
                if not invoice.origin or invoice_vals['origin'] not in invoice.origin.split(', '):
                    invoice_origin = [_f for _f in [invoice.origin, invoice_vals['origin']] if _f]
                ...
                if invoice_vals.get('name', False) and (not invoice.name or invoice_vals['name'] not in invoice.name.split(', ')):
                    invoice_name = [_f for _f in [invoice.name, invoice_vals['name']] if _f]
        ...
        invoice_obj.button_compute(cr, uid, list(invoices.values()), context=context, set_total=(inv_type in ('in_invoice', 'in_refund')))
