...
class TestReconciliation(TransactionCase):
    ...
    def test_balanced_customer_invoice(self):
        ...
        for move_line in move_line_ids:
            if move_line.account_id.id == self.account_usd_id:
                self.assertEqual(move_line.debit, 27.47)
                self.assertEqual(move_line.credit, 0.0)
                self.assertEqual(move_line.amount_currency, 42)
                self.assertEqual(move_line.currency_id.id, self.currency_usd_id)
                checked_line += 1
                continue
            if move_line.account_id.id == self.account_rcv_id:
                self.assertEqual(move_line.debit, 0.0)
                self.assertEqual(move_line.credit, 38.21)
                self.assertEqual(move_line.amount_currency, -50)
                self.assertEqual(move_line.currency_id.id, self.currency_swiss_id)
                checked_line += 1
                continue
            if move_line.account_id.id == self.account_fx_expense_id:
                self.assertEqual(move_line.debit, 10.74)
                self.assertEqual(move_line.credit, 0.0)
                checked_line += 1
                continue
    ...
    def test_balanced_supplier_invoice(self):
        ...
        for move_line in move_line_ids:
            if move_line.account_id.id == self.account_usd_id:
                self.assertEqual(move_line.debit, 0.0)
                self.assertEqual(move_line.credit, 27.47)
                self.assertEqual(move_line.amount_currency, -42)
                self.assertEqual(move_line.currency_id.id, self.currency_usd_id)
                checked_line += 1
                continue
            if move_line.account_id.id == self.account_rcv_id:
                self.assertEqual(move_line.debit, 38.21)
                self.assertEqual(move_line.credit, 0.0)
                self.assertEqual(move_line.amount_currency, 50)
                self.assertEqual(move_line.currency_id.id, self.currency_swiss_id)
                checked_line += 1
            ...
            if move_line.account_id.id == self.account_fx_income_id:
                self.assertEqual(move_line.debit, 0.0)
                self.assertEqual(move_line.credit, 10.74)
                checked_line += 1
                continue
        self.assertEqual(checked_line, 3)
