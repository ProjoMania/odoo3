...
def decode_header(message, header, separator=' '):
    return separator.join(map(decode, [_f for _f in message.get_all(header, []) if _f]))
...
class mail_thread(osv.AbstractModel):
    ...
    def read_followers_data(self, cr, uid, follower_ids, context=None):
        ...
        for follower in self.pool.get('res.partner').browse(cr, uid, follower_ids, context=context):
            ...
            is_uid = uid in [x.id for x in follower.user_ids]
            data = (follower.id,
                    follower.name,
    ...
    def create(self, cr, uid, values, context=None):
        ...
        self.message_auto_subscribe(cr, uid, [thread_id], list(create_values.keys()), context=context, values=create_values)
        ...
        if not context.get('mail_notrack'):
            tracked_fields = self._get_tracked_fields(cr, uid, list(values.keys()), context=track_ctx)
            if tracked_fields:
    ...
    def write(self, cr, uid, ids, values, context=None):
        ...
        # update followers
        self.message_auto_subscribe(cr, uid, ids, list(values.keys()), context=context, values=values)
    ...
    def message_track(self, cr, uid, ids, tracked_fields, initial_values, context=None):
        ...
        def format_message(message_description, tracked_values):
            ...
            for name, change in list(tracked_values.items()):
        ...
        for browse_record in self.browse(cr, uid, ids, context=context):
            ...
            if not context.get('mail_track_log_only'):
                for field, track_info in list(self._track.items()):
                    if field not in changes:
                        continue
                    for subtype, method in list(track_info.items()):
            ...
            if model_name and model_name != 'mail.thread':
                ...
                doc_names.update(
                    dict((ng_res[0], ng_res[1])
                         for ng_res in self.pool[model_name].name_get(cr, SUPERUSER_ID, list(aliases.keys()), context=context)))
    ...
    def _message_find_partners(self, cr, uid, message, header_fields=['From'], context=None):
        ...
        return [x for x in self._find_partner_from_emails(cr, uid, None, tools.email_split(s), context=context) if x]
    ...
    def message_post(self, cr, uid, thread_id, body='', subject=None, type='notification',
        ...
        if context.get('mail_post_autofollow') and thread_id and partner_ids:
            ...
            if context.get('mail_post_autofollow_partner_ids'):
                partner_to_subscribe = [item for item in partner_ids if item in context.get('mail_post_autofollow_partner_ids')]
    ...
    def message_auto_subscribe(self, cr, uid, ids, updated_fields, context=None, values=None):
        ...
        for pid, subtypes in list(new_followers.items()):
