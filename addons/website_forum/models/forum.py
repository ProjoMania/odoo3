...
class Forum(osv.Model):
    ...
    def _tag_to_write_vals(self, cr, uid, ids, tags='', context=None):
        ...
        for forum in self.browse(cr, uid, ids, context=context):
            post_tags = []
            existing_keep = []
            for tag in [_f for _f in tags.split(',') if _f]:
...
class Post(osv.Model):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if any(key not in ['state', 'active', 'is_correct', 'closed_uid', 'closed_date', 'closed_reason_id'] for key in list(vals.keys())) and any(not post.can_edit for post in posts):
            raise KarmaError('Not enough karma to edit a post.')
