...
class BuckarooForm(BuckarooCommon):
    ...
    def test_10_Buckaroo_form_render(self):
        ...
        form_values = {
            ...
            'brq_test': 'True',
            'Brq_return': '%s' % urllib.parse.urljoin(self.base_url, BuckarooController._return_url),
            'Brq_returncancel': '%s' % urllib.parse.urljoin(self.base_url, BuckarooController._cancel_url),
            'Brq_returnerror': '%s' % urllib.parse.urljoin(self.base_url, BuckarooController._exception_url),
            'Brq_returnreject': '%s' % urllib.parse.urljoin(self.base_url, BuckarooController._reject_url),
            'Brq_culture': 'en-US',
        }
    ...
    def test_20_buckaroo_form_management(self):
        ...
        buckaroo_post_data = {
            ...
            'BRQ_SERVICE_PAYPAL_PAYEREMAIL': 'fhe@openerp.com',
