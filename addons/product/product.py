    ...
    def _product_template_price(self, cr, uid, ids, name, arg, context=None):
        ...
        if pricelist:
            ...
            if isinstance(pricelist, int):
                ...
                qtys = [(x, quantity, partner) for x in products]
        ...
        for tmpl_id in tmpl_ids:
            ...
            for variant_id in variant_alone:
                ...
                for product_id in tmpl_id.product_variant_ids:
                    if variant_id.id not in list(map(int, product_id.attribute_value_ids)):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if 'active' in vals and not vals.get('active'):
            ...
            for product in self.browse(cr, uid, ids, context=ctx):
                product_ids += list(map(int, product.product_variant_ids))
...
class product_product(osv.osv):
    ...
    def name_get(self, cr, user, ids, context=None):
        ...
        for product in self.browse(cr, SUPERUSER_ID, ids, context=context):
            ...
            sellers = []
            if partner_ids:
                sellers = [x for x in product.seller_ids if x.name.id in partner_ids]
            if sellers:
                for s in sellers:
