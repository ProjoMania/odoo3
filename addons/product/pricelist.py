...
class product_pricelist(osv.osv):
    ...
    def price_get_multi(self, cr, uid, ids, products_by_qty_by_partner, context=None):
        return dict((key, dict((key, price[0]) for key, price in list(value.items()))) for key, value in list(self.price_rule_get_multi(cr, uid, ids, products_by_qty_by_partner, context=context).items()))
    ...
    def price_rule_get_multi(self, cr, uid, ids, products_by_qty_by_partner, context=None):
        ...
        for pricelist in self.browse(cr, uid, ids, context=context):
            ...
            for product_id,price in list(subres.items()):
                results.setdefault(product_id, {})
                results[product_id][pricelist.id] = price
        return results

    def _price_get_multi(self, cr, uid, pricelist, products_by_qty_by_partner, context=None):
        return dict((key, price[0]) for key, price in list(self._price_rule_get_multi(cr, uid, pricelist, products_by_qty_by_partner, context=context).items()))
    ...
    def price_get(self, cr, uid, ids, prod_id, qty, partner=None, context=None):
        return dict((key, price[0]) for key, price in list(self.price_rule_get(cr, uid, ids, prod_id, qty, partner=partner, context=context).items()))
