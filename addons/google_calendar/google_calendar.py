...
class google_calendar(osv.AbstractModel):
    ...
    def create_an_event(self, cr, uid, event, context=None):
        ...
        url = "/calendar/v3/calendars/%s/events?fields=%s&access_token=%s" % ('primary', urllib.parse.quote('id,updated'), self.get_token(cr, uid, context))
...
class calendar_event(osv.Model):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if (set(vals.keys()) & sync_fields) and 'oe_update_date' not in list(vals.keys()) and 'NewMeeting' not in context:
