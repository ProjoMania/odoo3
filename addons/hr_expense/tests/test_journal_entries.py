...
class TestCheckJournalEntry(TransactionCase):
    ...
    def test_journal_entry(self):
        ...
        self.assertEqual(self.expense.state, 'done', 'Expense is not in Waiting Payment state')
        ...
        for line in self.expense.account_move_id.line_id:
            if line.credit:
                self.assertEqual(line.credit, 700.00, 'Expense Payable Amount is not matched for journal item')
            else:
                if line.tax_code_id:
                    self.assertEqual(line.debit, 636.36, 'Tax Amount is not matched for journal item')
                else:
                    self.assertEqual(line.debit, 63.64, 'Tax Base Amount is not matched for journal item')
