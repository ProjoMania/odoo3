...
#
##############################################################################
__name__ = "Renombrar impuestos, códigos de impuestos y posiciones fiscales"
...
def rename_taxes(cr):
    ...
    for old_description, new_description in tax_mapping.items():
        sql = """
        UPDATE account_tax
        SET description=%s
        WHERE description=%s"""
...
def change_refunds_tax_codes(cr):
    ...
    for record in cr.fetchall():
        company_id = record[0]
        for old_tax_code, new_tax_code in refund_tax_codes.items():
            cr.execute(
