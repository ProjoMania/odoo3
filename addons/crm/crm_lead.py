...
class crm_lead(format_address, osv.osv):
    ...
    def _resolve_section_id_from_context(self, cr, uid, context=None):
        ...
        if context is None:
            context = {}
        if type(context.get('default_section_id')) in (int, int):
            return context.get('default_section_id')
        if isinstance(context.get('default_section_id'), str):
    ...
    def case_mark_lost(self, cr, uid, ids, context=None):
        ...
        for stage_id, lead_ids in list(stages_leads.items()):
    ...
    def _merge_data(self, cr, uid, ids, oldest, fields, context=None):
        ...
        def _concat_all(attr):
            return '\n\n'.join([x for x in [getattr(opp, attr) or '' for opp in opportunities if hasattr(opp, attr)] if x])
    ...
    def merge_opportunity(self, cr, uid, ids, user_id=False, section_id=False, context=None):
        ...
        opportunities = list(map(itemgetter(1), sequenced_opps))
