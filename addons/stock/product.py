...
class product_product(osv.osv):
    ...
    def _get_domain_locations(self, cr, uid, ids, context=None):
        ...
        if context.get('location', False):
            ...
            if context.get('warehouse', False):
                if isinstance(context['warehouse'], int):
                    wids = [context['warehouse']]
                elif isinstance(context['warehouse'], str):
    ...
    def _product_available(self, cr, uid, ids, field_names=None, arg=False, context=None):
        ...
        moves_in = dict([(x['product_id'][0], x['product_qty']) for x in moves_in])
    ...
    def _search_qty_available(self, cr, uid, operator, value, context):
        ...
        quants = self.pool.get('stock.quant').read_group(cr, uid, domain_quant, ['product_id', 'qty'], ['product_id'], context=context)
        quants = dict([(x['product_id'][0], x['qty']) for x in quants])
        quants = dict((k, v) for k, v in quants.items() if eval(str(v) + operator + str(value)))
        return(list(quants))
