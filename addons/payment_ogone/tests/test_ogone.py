...
class OgonePayment(PaymentAcquirerCommon):
    ...
    def test_10_ogone_form_render(self):
        ...
        form_values = {
            ...
            'ACCEPTURL': '%s' % urllib.parse.urljoin(self.base_url, OgoneController._accept_url),
            'DECLINEURL': '%s' % urllib.parse.urljoin(self.base_url, OgoneController._decline_url),
            'EXCEPTIONURL': '%s' % urllib.parse.urljoin(self.base_url, OgoneController._exception_url),
            'CANCELURL': '%s' % urllib.parse.urljoin(self.base_url, OgoneController._cancel_url),
        }
