...
class PaymentTxOgone(osv.Model):
    ...
    def ogone_s2s_create_alias(self, cr, uid, id, values, context=None):
        ...
        request = urllib.request.Request(tx.acquirer_id.ogone_afu_agree_url, urlencode(tx_data))
    ...
    def ogone_s2s_execute(self, cr, uid, id, values, context=None):
        ...
        request = urllib.request.Request(tx.acquirer_id.ogone_direct_order_url, urlencode(tx_data))
        result = urllib.request.urlopen(request).read()
        ...
        while not tx_done or tries > 0:
            ...
            if status in VALID_TX:
                ...
                request = urllib.request.Request(query_direct_url, urlencode(query_direct_data))
                result = urllib.request.urlopen(request).read()
