...
class mrp_production(osv.osv):
    ...
    def _calculate_qty(self, cr, uid, production, product_qty=0.0, context=None):
        ...
        for product_id in list(scheduled_qty.keys()):
            ...
            for move in production.move_lines:
                ...
                for quant, quant_qty in quants:
                    if quant:
                        ...
                        if not product_id in list(dicts.keys()):
                            dicts[product_id] = {lot_id: quant_qty}
                        elif lot_id in list(dicts[product_id].keys()):
                            dicts[product_id][lot_id] += quant_qty
                        else:
                            dicts[product_id][lot_id] = quant_qty
                        qty -= quant_qty
        ...
        consume_lines = []
        for prod in list(dicts.keys()):
            for lot, qty in list(dicts[prod].items()):
    ...
    def action_confirm(self, cr, uid, ids, context=None):
        ...
        uncompute_ids = [x for x in [not x.product_lines and x.id or False for x in self.browse(cr, uid, ids, context=context)] if x]
