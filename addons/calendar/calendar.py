...
class calendar_attendee(osv.Model):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if not vals.get("email") and vals.get("cn"):
            ...
            email = [x for x in cnval if x.__contains__('@')]
...
class calendar_alarm_manager(osv.AbstractModel):
    ...
    def get_next_mail(self, cr, uid, context=None):
        ...
        if cron.interval_type not in list(interval_to_second.keys()):
        ...
        for curEvent in self.pool.get('calendar.event').browse(cr, uid, list(all_events.keys()), context=context):
...
class ir_model(osv.Model):
    ...
    def read(self, cr, uid, ids, fields=None, context=None, load='_classic_read'):
        new_ids = isinstance(ids, (str, int)) and [ids] or ids
        if context is None:
            context = {}
        ...
        return isinstance(ids, (str, int)) and data[0] or data
...
class calendar_event(osv.Model):
        ...
        if values.get('start_datetime') or values.get('start_date') or values.get('start') \
            ...
            for fld in ('start', 'stop'):
                if values.get('%s_%s' % (fld, key)) or values.get(fld):
                    ...
                    if fld not in list(values.keys()):
    ...
    def create_attendees(self, cr, uid, ids, context=None):
        ...
        for event in self.browse(cr, uid, ids, context):
            ...
            partner_ids_to_remove = [x for x in set(all_part_attendee_ids + new_att_partner_ids) - set(all_partner_ids)]
    ...
    def get_recurrent_ids(self, cr, uid, event_id, domain, order=None, context=None):
        ...
        if order_fields:
            uniq = lambda it: list(collections.OrderedDict((id(x), x) for x in it).values())
    ...
    def compute_rule_string(self, data):
        ...
        def get_week_string(freq, data):
            ...
            if freq == 'weekly':
                byday = [x.upper() for x in [x for x in data if data.get(x) and x in weekdays]]
                if byday:
        ...
        for event_id in ids:
            ...
            if any(field in values for field in ['recurrency', 'end_type', 'count', 'rrule_type', 'start', 'stop']):
                for event in self.browse(cr, uid, real_ids, context=context):
                    if event.recurrency and event.end_type in ('count', str('count')):
    ...
    def read(self, cr, uid, ids, fields=None, context=None, load='_classic_read'):
        ...
        if isinstance(ids, (str, int)):
            ...
            select = ids
        select = [(x, calendar_id2real_id(x)) for x in select]
        result = []
        ...
        real_data = dict(list(zip([x['id'] for x in real_data], real_data)))
...
class mail_message(osv.Model):
    ...
    def _find_allowed_model_wise(self, cr, uid, doc_model, doc_dict, context=None):
        ...
        if doc_model == 'calendar.event':
            ...
            for virtual_id in self.pool[doc_model].get_recurrent_ids(cr, uid, list(doc_dict.keys()), [], order=order, context=context):
