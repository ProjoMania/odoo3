...
class xml_decl(osv.TransientModel):
    ...
    def _get_lines(self, cr, uid, ids, decl_datas, company, dispatchmode=False,
        ...
        for linekey in entries:
            ...
            self._set_Dim(item, 'EXTXVAL', str(round(amounts[0], 0)).replace(".", ","))
            self._set_Dim(item, 'EXWEIGHT', str(round(amounts[1], 0)).replace(".", ","))
            self._set_Dim(item, 'EXUNITS', str(round(amounts[2], 0)).replace(".", ","))
