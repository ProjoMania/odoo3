...
def _str2time(cre):
    ...
    if not cre:
        return time.time()
    frac = 0.0
    if isinstance(cre, str) and '.' in cre:
        fdot = cre.find('.')
        frac = float(cre[fdot:])
        cre = cre[:fdot]
...
class node_dir(node_database):
    ...
    def __init__(self, path, parent, context, dirr, dctx=None):
        ...
        self.unixperms = 0o40000 | _uid2unixperms(self.uidperms, dirr and dirr.user_id)
        if dctx:
...
class node_res_obj(node_class):
    ...
    def __init__(self, path, dir_id, parent, context, res_model, res_bo, res_id=None):
        ...
        self.unixperms = 0o40000 | _uid2unixperms(self.uidperms, True)
        ...
        if res_bo:
            ...
            dc2['this'] = res_bo
            for fld,expr in list(self.dctx_dict.items()):
                try:
